...
module.exports = {
  init: function () {
    ...
    _panel.port.on('pause-clicked', () => event.emit(this, 'pause-action'))
    _panel.port.on('resume-clicked', () => event.emit(this, 'resume-action'))
    _panel.port.on('link-clicked', url => tabs.open(url))
