...
let TabsManager = {
    init: function () {
        ...
        tabs.on('deactivate', tab => lastActivity[tab.id] = Date.now());
    ...
    pause: function () {
        paused = true;
        console.info('Paused auto-closing');
    },

    resume: function () {
        paused = false;
        Object.keys(lastActivity).forEach( id => lastActivity[id] = Date.now() );
        console.info('Resumed auto-closing');
    }
};
